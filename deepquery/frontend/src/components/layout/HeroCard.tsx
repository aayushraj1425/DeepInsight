...
export type HeroStatus = "idle" | "running" | "done" | "error"
...
const STATUS_COPY: Record<HeroStatus, { label: string; className: string; Icon: typeof Loader2 }> = {
  idle: {
    label: "Ready",
    className: "border-slate-600/40 bg-slate-500/10 text-slate-200",
    Icon: Loader2,
  },
  running: {
    label: "Running",
    ...
    <section className="rounded-lg border border-slate-800 bg-[#11151b] p-5">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0 flex-1">
          ...
          <h1 className="mt-4 max-w-3xl text-2xl font-semibold leading-tight tracking-tight text-white">{title}</h1>
          <p className="mt-3 max-w-4xl text-sm leading-7 text-slate-400">{summary}</p>
        </div>

        <div className="grid shrink-0 grid-cols-2 gap-2 sm:grid-cols-4 lg:w-[460px] lg:grid-cols-4">
          {metrics.slice(0, 8).map((metric) => (
            <div key={metric.label} className="rounded-md border border-slate-800 bg-slate-950/50 px-3 py-3">
              ...
              <div className="mt-1 truncate font-mono text-lg font-semibold text-slate-100">{metric.value}</div>
            </div>
          ))}
        </div>
      </div>
